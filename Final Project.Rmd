---
title: "Driver's Traffic Fatality and How Could We Survive"
author: "Wenyi Lin"
...
## Introduction
Every day, nearly 100 people die from vehicle-related accidents. From 2010 to 2015, there are 2912 people died in 2699 fatal traffic accidents in Maryland. Basically, traffic accidents are unavoidable, however, it is possible that we face the fact and protect ourselves. Thus, we would like to know what factors could have impact on a severe accident which might cause fatality, especially for a driver. And also, if a possibaly fatal accident happens, do we have any chance to prolong our lives, and even, save our lives.

Fatal risks include personal chracters (age, sex, position)$^2$, time factors (season, time of the day), driving behaviours (drunk, safety measurement) and environmental dectription (location)$^3$. Each of these factors could be a potential threat to our safety and they could also work interactively. Here we fit a logistic model$^2$ to figure out their causality and relationship.

Once a serious traffic accident happens, the response and rescue from hospitals would be the last hope to save the live. Response time and arriving time at the scene$^4$ would be the first concern to construct a survival model.

## Data Description
Data was compiled using the 2010-2015 Fatality Analysis Reporting System (FARS) data$^1$. We use Maryland fatality data extracted from the big data set to analyze risks in fatal traffic accidents within this state. Missing data related with age, death time was eliminated from analysis. Due to large volumn of missing data with medical service time records, thus, we applied survival analysis on nationwide dataset and used available medical sevice time variables.
...
library(Hmisc)
#working dir
#data.folder<-"/Users/wendylin/Documents/AD/traffic_fatality/data/" #local dir
#load(paste0(data.folder,"maryland_fatal_data.RData"))
load("data/maryland_fatal_data.RData")
...
We made plots to show the effect of possible risk factors. Drivers are relatively to be threatened by death among all accident-relevant types of people(passenger, pedestrian, Bicyclist, other). Basically, death rate is higher when the accident is happened at rush hour (7-10 am, 4-7pm) on weekdays between May to October. Accidents happened at rural area are likely to have dead cases compared with that at urban area. And apparently, using seat belt could decrease the death probability. Based on our findings, we would like to consider cases only include over-16-year individuals to confirm the possibility of being a driver.
...
With plotting the relationship between age and fatality (supplemental), we find that the continuous variable age could be divided into three categories. Natural spline was used on age factor in order to allow a smooth relationship between age and outcome. By cross validation, we get the AUC for simple model is 0.635.
...
Actually, the analysis is incomplete to some extent. Fisrt of all, it is better to include all accident data over 5 years in Maryland to make inference about fatality rate, probabily with imputation method. Secondly, more factors could be included in the model to improve the accuracy of the predictive model.

### 2. Survival Analysis
A fatal accident could always happen no matter how we try to avoid it. In this situation, it is critical prolong the survival time or even save lives. It is believed that medical response time could affect survival time after traffic accidents$^4$. 
...
library(survival)
library(survminer)
load("data/nation_survival_data.RData")
...
Lag time is the period between the time of the crash and this person’s time of death. Figure 2A shows the fatality rate over time. This fatality curve, or survival probability curve over time, S(t), indicates that approximately `r round(sum(death_lag[1:2])/sum(death_lag),3)` of fatalities occurring within 6 hours occurred within 100 minutes of the crash (including `r round(death_lag[1]/sum(death_lag),3)` instant deaths before time = 10). Then, we plot fatality curve for stratified dataset based on notification time and arrival time. And it turns out that arrival time has a more significant effect on fatality rate. Figure 2B indicates that early arrival time (arrival time $<$ 10) could help with decrease the proportion of instant death.
...
```{r, message=FALSE, warning=FALSE, include=FALSE}
fit1 = coxph(Surv(lag)~ early_arrive+RUR_URB, data = arr.not.data[arr.not.data$lag<=400,],
             ties = "breslow")
summary(fit1)
...
The model included two factors treated as binary variables: arrival time (later than 10 minutes versus earlier arrival) and crash location (rural versus urban)$^4$. From the results, we conclude that later arrival time could carry a 2.14% (0.38%,3.92%) relatively higher fatality hazard than earlier group. Moreover, hazard rate for urban area is 0.8439 (0.8295,0.8586) of that for rural areas, which means longer survival time in cities.

Now we could say that earlier arrival of emergency service helps with increase of survival probability since most of the fatal case happens very quickly. However, this simple survival analysis could not include all of the imformation to estimate the survival rate. And unfortunately, the time of death after injury and emergency service arrival may be inconsistently recorded. A thorough analysis of the data would be required if we want a more precise result.

## Conclusion
As for fatal traffic cases, many factors would influence the death of a person involved. Generally speaking, drivers in the accident are the most vulnerable people and the situation could be even worse if they don't wear a seat belt. The fatality rate would increase when roads get crowded, such as rush hour, week day, and relatively warm seasons. Based on that, drivers should be more careful when there is a relatived busy traffic on the road. And also, in-time emergency medical service could help to decrease the probability of early mortality and accordingly, create chances to save lives. 

## Reference
[1]. U.S Departmetn of Transportation. [2015 Traffic Fatalities Data Has Just Been Released: A Call to Action to Download and Analyze](https://www.transportation.gov/fastlane/2015-traffic-fatalities-data-has-just-been-released-call-action-download-and-analyze) 

[2]. Lui, Kung-Jong, et al. "An application of a conditional logistic regression to study the effects of safety belts, principal impact points, and car weights on drivers' fatalities." Journal of Safety Research 19.4 (1989): 197-203.

[3]. Clark, David E., and Brad M. Cushing. "Rural and urban traffic fatalities, vehicle miles, and population density." Accident Analysis & Prevention 36.6 (2004): 967-972.

[4]. Wu, Jingshu, Matthew Craig, and Anders Longthorne. "Updated Estimates of Potential Traffic Fatality Reductions With Automatic Collision Notification." 24th International Technical Conference on the Enhanced Safety of Vehicles (ESV). No. 15-0269. 2015.
